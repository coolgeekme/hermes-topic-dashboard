    ...
    <div className="flex-1 overflow-y-auto p-6 lg:p-8" style={{ backgroundColor: 'var(--bg-deep)' }}>
      <div className="max-w-[1440px] mx-auto flex flex-col gap-6">
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div>
            <h1 className="text-[28px] font-bold" style={{ fontFamily: "'Hanken Grotesk', sans-serif", color: 'var(--text)' }}>分析與任務</h1>
            <p className="text-[15px] mt-1" style={{ fontFamily: "'Inter', sans-serif", color: 'var(--text-muted)' }}>監控你的 AI 對話活動。</p>
          </div>
        </div>

        {/* Stats Row */}
        <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
          <StatCard label="總訊息數" value={totalMsgs.toLocaleString()} icon="💬" trend="+12%（本週）" positive />
          <StatCard label="總主題數" value={totalTopics.toString()} icon="📁" trend={`${totalTopics} 個進行中`} positive />
          <StatCard label="預估字元數" value={(totalTokens / 1e6).toFixed(1) + 'M'} icon="⚡" trend={`平均每主題 ${avgMsgsPerSession} 則訊息`} positive />
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="lg:col-span-2 flex flex-col gap-5">
            ...
            <div className="rounded-xl border p-5 flex flex-col" style={{ backgroundColor: 'var(--card-bg)', borderColor: 'var(--card-border)' }}>
              <div className="flex justify-between items-center mb-4">
                <h3 className="text-[16px] font-semibold" style={{ fontFamily: "'Hanken Grotesk', sans-serif" }}>主題活躍度</h3>
                <button className="text-[11px]" style={{ fontFamily: "'Geist', monospace", color: 'var(--text-muted)' }}>本週</button>
              </div>
            ...
            {/* Donut chart */}
            <div className="rounded-xl border p-5 flex flex-col" style={{ backgroundColor: 'var(--card-bg)', borderColor: 'var(--card-border)' }}>
              <h3 className="text-[16px] font-semibold mb-4" style={{ fontFamily: "'Hanken Grotesk', sans-serif" }}>平台分佈</h3>
              <div className="flex items-center gap-6">
                <div className="w-36 h-36 rounded-full relative flex items-center justify-center flex-shrink-0"
                  ...
                  <div className="w-24 h-24 rounded-full flex flex-col items-center justify-center" style={{ backgroundColor: 'var(--card-bg)' }}>
                    ...
                    <span className="text-[9px]" style={{ fontFamily: "'Geist', monospace", color: 'var(--text-muted)' }}>主題</span>
                  </div>
                </div>
          ...
          <div className="rounded-xl border flex flex-col max-h-[600px]" style={{ backgroundColor: 'var(--card-bg)', borderColor: 'var(--card-border)' }}>
            <div className="p-4 border-b flex justify-between items-center sticky top-0 rounded-t-xl z-10"
              ...
              <h3 className="text-[16px] font-semibold" style={{ fontFamily: "'Hanken Grotesk', sans-serif" }}>最近主題</h3>
            </div>
            <div className="flex-1 overflow-y-auto p-2">
              {recentTopics.map((topic, i) => {
                ...
                return (
                  ...
                  >
                    <div className="w-2 h-2 rounded-full mt-2 flex-shrink-0" style={{ backgroundColor: dotColor }} title={priority === 'high' ? '高優先' : priority === 'medium' ? '中優先' : '低優先'} />
